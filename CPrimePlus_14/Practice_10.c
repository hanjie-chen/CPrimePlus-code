...
void Init_Function_point(int (* pf[Function])(struct Giant_Airplane_Seat *, int)); // 初始化函数指针数组
void Init_Airplane(struct Giant_Airplane_Seat (*ps)[Seat_Count], int flight); // 初始化结构数组
int Print_Empty_Seats(struct Giant_Airplane_Seat *, int); // 打印空余位置数量
int Print_Empty_Seats_List(struct Giant_Airplane_Seat *, int); // 打印空余座位列表
int Print_All_Seats(struct Giant_Airplane_Seat *, int); // 打印所有座位
...
int main(void)
{
    ...
    char * flight_sequence[Flight] = {"102", "311", "444", "519"};
    int (* pf[Function])(struct Giant_Airplane_Seat *, int); // 函数指针数组

//    初始化四个航班
    Init_Airplane(fleet, Flight);
//    初始化函数指针数组
    Init_Function_point(pf);
...
//        找到并确认顾客需要的航班
        ...
        while (1)
        {
            ...
            if ('f' == choose)
                break;
            pf[choose - 'a'](fleet[location], Seat_Count);
        }
    }
...
void Init_Function_point(int (* pf[Function])(struct Giant_Airplane_Seat *, int))
{
    pf[0] = Print_Empty_Seats;
    pf[1] = Print_Empty_Seats_List;
    pf[2] = Print_All_Seats;
    pf[3] = Assign_Seats;
    pf[4] = Cancel_Reserve;
}
int Print_Empty_Seats(struct Giant_Airplane_Seat * ps, int seat_count)
{
    int count = 0;
...
int Print_Empty_Seats_List(struct Giant_Airplane_Seat * ps, int seats_count)
{
...
int Print_All_Seats(struct Giant_Airplane_Seat * ps, int seats_count)
{
    for (int i = 0; i < seats_count; ++i)
    {
        if (ps[i].is_reserve)
