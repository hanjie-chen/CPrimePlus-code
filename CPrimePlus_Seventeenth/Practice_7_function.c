...
{
    return (TreeItemCount(point_tree) == MAX_TREE) ? true : false;
}
...
bool AddTreeNode(const Item * temp, Tree * point_tree)
{
    ...
    Tree_Node * node_temp;
    Tree_Node * in_tree;
    in_tree = SeekItem(temp, point_tree).children_node;
    if (in_tree != NULL)
    {
//        fputs("find the same item! Add the ", stdout);
        in_tree->item.word_count++;
        return true;
    }
...
bool InTree(Item * temp,const Tree * point_tree)
{
    ...
    {
        result = true;
        temp->word_count = in_tree->item.word_count;
    }
...
static bool ToLeft(const Item * item_first,const Item * item_second)
{
    ...
    record = strcmp(item_first->word, item_second->word);
    if (record < 0)
        return true;
    else
        return false;
